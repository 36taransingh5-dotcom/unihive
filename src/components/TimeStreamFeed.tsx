...
import { TimelineWrapper, TimelinePin } from '@/components/TimelineWrapper';
    ...
    <TimelineWrapper>
      <div className="space-y-6">
        ...
        {futureGroupOrder.map((group) => {
          ...
          return (
            <section key={group}>
              <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3 pl-10">
                {timeGroupLabels[group]}
              </h2>
              <div className="space-y-3">
                {groupEvents.map((event, idx) => (
                  <TimelinePin key={event.id}>
        ...
        {hasPastEvents && (
          <Collapsible open={isPastOpen} onOpenChange={setIsPastOpen}>
            <CollapsibleTrigger className="w-full pl-10">
